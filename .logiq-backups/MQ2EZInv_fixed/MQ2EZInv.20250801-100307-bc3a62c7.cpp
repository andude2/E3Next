...
void EZInvCommand(PlayerClient* pChar, const char* szLine) {
	...
	if (args.empty() || args == "ui") {
		...
		// NEW: Force inventory sync with all peers
		if (g_actorManager && g_actorManager->IsInitialized()) {
			...
			WriteChatf("[MQ2EZInv] Forcing inventory sync with all peers");
		}
		else {
			WriteChatf("[MQ2EZInv] Actor system not initialized - cannot sync");
		}
	}
	else if (args == "peers") {
		...
		WriteChatf("  /ezinv sync - Force sync with all peers");
