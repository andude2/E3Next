...
E3NextDirectNetworking::E3NextDirectNetworking()
    : m_isInitialized(false), m_lastPublish(std::chrono::steady_clock::now()),
      m_hPipe(INVALID_HANDLE_VALUE), m_pipeConnected(false)
{
}
...
bool E3NextDirectNetworking::Initialize()
{
    ...
    // Try to create initial pipe connection
    if (!CreateAndConnectPipe()) {
        WriteChatf("[MQ2EZInv] Warning: Could not connect to named pipe - will retry later");
        // Don't fail initialization, just retry later
    }
...
bool E3NextDirectNetworking::PublishInventory(const InventoryData& inventory)
{
    ...
    try {
        ...
        // Ensure pipe connection and send data
        if (!EnsurePipeConnection()) {
            WriteChatf("[MQ2EZInv] Could not connect to pipe for publishing");
            return false;
        }

        // Send inventory data via named pipe
        if (WriteToPipe(jsonData)) {
            UpdateLastPublishTime();
            WriteChatf("[MQ2EZInv] Published inventory data for %s (%d chars) via named pipe", 
                      inventory.characterName.c_str(), static_cast<int>(jsonData.length()));
            return true;
        } else {
            WriteChatf("[MQ2EZInv] Failed to write to named pipe");
            return false;
        }
    }
...
void E3NextDirectNetworking::ProcessUpdates()
{
    ...
    // Ensure pipe connection is maintained
...
void E3NextDirectNetworking::RequestAllInventories()
{
    ...
    // Send request message through pipe
    std::string requestJson = R"({"type":"request_inventory","character":")" + m_characterName + R"("})";
    
    if (EnsurePipeConnection() && WriteToPipe(requestJson)) {
        WriteChatf("[MQ2EZInv] Sent inventory request via named pipe");
    } else {
        WriteChatf("[MQ2EZInv] Failed to send inventory request via pipe");
    }
}
...
bool E3NextDirectNetworking::CreateAndConnectPipe()
{
    ...
    // Create the named pipe as a server (E3Next will connect to it as a client)
    m_hPipe = CreateNamedPipeA(
        m_pipeName.c_str(),               // Pipe name
        PIPE_ACCESS_OUTBOUND,             // Direction: outbound (MQ2EZInv writes)
        PIPE_TYPE_MESSAGE | PIPE_READMODE_MESSAGE | PIPE_WAIT, // Type and mode
        1,                               // Max instances
    ...
    if (m_hPipe == INVALID_HANDLE_VALUE) {
        WriteChatf("[MQ2EZInv] Failed to create named pipe: %d", GetLastError());
        return false;
    }

    WriteChatf("[MQ2EZInv] Named pipe created: %s", m_pipeName.c_str());

    // Wait for E3Next to connect (non-blocking check)
    DWORD dwMode = PIPE_TYPE_MESSAGE | PIPE_READMODE_MESSAGE | PIPE_WAIT;
    SetNamedPipeHandleState(m_hPipe, &dwMode, NULL, NULL);

    // Try to connect (brief wait)
    if (ConnectNamedPipe(m_hPipe, NULL) || GetLastError() == ERROR_PIPE_CONNECTED) {
        m_pipeConnected = true;
        WriteChatf("[MQ2EZInv] Connected to E3Next via named pipe");
        return true;
    } else {
        DWORD error = GetLastError();
        if (error == ERROR_PIPE_LISTENING) {
            WriteChatf("[MQ2EZInv] Waiting for E3Next to connect to pipe...");
        } else {
            WriteChatf("[MQ2EZInv] Failed to connect to pipe: %d", error);
        }
        ClosePipe();
        return false;
    }
...
bool E3NextDirectNetworking::WriteToPipe(const std::string& jsonData)
{
    ...
    DWORD bytesWritten;
    if (WriteFile(m_hPipe, jsonData.c_str(), static_cast<DWORD>(jsonData.length()), &bytesWritten, NULL)) {
        // Flush the pipe to ensure data is sent
        FlushFileBuffers(m_hPipe);
        return true;
    } else {
        DWORD error = GetLastError();
        WriteChatf("[MQ2EZInv] Failed to write to pipe: %d", error);
...
{
    return m_hPipe != INVALID_HANDLE_VALUE && m_pipeConnected;
}
...
void E3NextDirectNetworking::ProcessReceivedInventoryData(const std::string& characterName, const std::string& jsonData)
{
    try {
        ...
        if (inventory.Deserialize(jsonData)) {
